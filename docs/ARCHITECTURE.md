...
A finished background task is a distinct case. The automation inbox injects a hidden message tagged with `sandAutomationCompletionId`. Native child revival uses another path whose stock formatter discards dispatch identity. The adapter wraps that exact formatter to retain a hidden marker derived from the original child and tool-call/request IDs. It preserves the stock result text and does not create an identity when those durable IDs are missing. The runtime also accepts the exact native hidden-completion envelope when a host request ID survives message conversion. It removes internal markers before forwarding the completion to the active provider and treats the completion as a new delivery boundary inside the existing user turn. This allows the child result to reach chat without replaying the earlier “subagent started” response. A durable signature combines that completion ID with later non-delivery tool-result IDs. It is claimed under the per-Bot lock before inference, expires after a bounded interval, and is cleared by reset. Sequential or concurrent host replays run once while controls and genuinely new tool rounds still proceed.
