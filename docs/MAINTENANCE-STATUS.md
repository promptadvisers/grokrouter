...
- Native memory extraction and periodic episode summaries receive separate text-task markers at their exact host executor calls. Memory extraction retains its system instructions, exposes no chat tools, returns text directly to the host, and does not alter saved chat threads or completion receipts. Explicit native maintenance sessions retain Grok's original inference backend and structured-text contract. The fdbae51 live audit exposed this ancillary-task defect. Revision 65254d4 passed the corrected memory-extraction probe with both providers and zero tools or router maintenance errors; complete final acceptance remains pending.

- Standalone literal replies normalize a complete printed SendToUser wrapper only when its text exactly matches the requested literal. The wrapper is decoded as text and never executed; unoffered native calls on literal/greeting turns are rejected. The direct and cursor-brokered SendToUser envelope forms are covered. The cc1c849 live probe returned the exact literal with a positive normalization audit and no outgoing tools.
...
- GitHub Mac and Windows builds and CodeQL all passed for literal-delivery revision `d942423` (CI run `34334386961`, CodeQL run `34334386795`).
