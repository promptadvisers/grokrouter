...
- Native child wake-up messages retain their original dispatch IDs through the adapter's hidden completion marker; automation inbox messages retain their explicit completion IDs. Identical result text does not merge distinct completion requests. Request-ID-only recognition failed live, and the dispatch-marker revision requires retesting.
